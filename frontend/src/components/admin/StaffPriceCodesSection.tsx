...
import type { StaffPriceCode, StaffPriceCodeFormData, ServiceType } from '@/types/serviceType'
...
interface StaffPriceCodesSectionProps {
...
export function StaffPriceCodesSection({ staffProfileId }: StaffPriceCodesSectionProps) {
  const { t } = useTranslation(['admin', 'common'])
  const { toast } = useToast()
  const queryClient = useQueryClient()
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [editingPriceCode, setEditingPriceCode] = useState<StaffPriceCode | null>(null)
  const [deleteConfirmId, setDeleteConfirmId] = useState<number | null>(null)

  // Fetch staff price codes
  const { data: priceCodes, isLoading: isLoadingPriceCodes } = useQuery({
    queryKey: staffPriceCodeKeys.listByStaff(staffProfileId),
    queryFn: () => staffPriceCodesApi.listByStaff(staffProfileId),
  ...
  const createMutation = useMutation({
    mutationFn: (data: StaffPriceCodeFormData) => staffPriceCodesApi.create(staffProfileId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: staffPriceCodeKeys.listByStaff(staffProfileId) })
      toast({ title: t('admin:staffPriceCodes.createSuccess') })
      handleCloseModal()
    },
    onError: () => {
      toast({
        ...
        description: t('admin:staffPriceCodes.createError'),
      })
    },
  })
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Partial<StaffPriceCodeFormData> }) =>
      staffPriceCodesApi.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: staffPriceCodeKeys.listByStaff(staffProfileId) })
      toast({ title: t('admin:staffPriceCodes.updateSuccess') })
      handleCloseModal()
    },
    onError: () => {
      toast({
        variant: 'destructive',
        title: t('common:error'),
        description: t('admin:staffPriceCodes.updateError'),
      })
    },
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => staffPriceCodesApi.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: staffPriceCodeKeys.listByStaff(staffProfileId) })
      toast({ title: t('admin:staffPriceCodes.deleteSuccess') })
    ...
    onError: () => {
      toast({
        variant: 'destructive',
        title: t('common:error'),
        description: t('admin:staffPriceCodes.deleteError'),
      })
    },
  })
  ...
  const toggleActiveMutation = useMutation({
    mutationFn: (id: number) => staffPriceCodesApi.toggleActive(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: staffPriceCodeKeys.listByStaff(staffProfileId) })
    },
  })
  ...
  const handleOpenEditModal = (priceCode: StaffPriceCode) => {
  ...
  const onSubmit = form.handleSubmit((data) => {
    const formData: StaffPriceCodeFormData = {
  ...
  // Get service types not yet assigned to this staff member
  const availableServiceTypes = serviceTypes?.filter(
    (st: ServiceType) =>
      st.is_active &&
      (!priceCodes?.some((pc: StaffPriceCode) => pc.service_type_id === st.id) || editingPriceCode?.service_type_id === st.id)
  )
  ...
  if (isLoadingPriceCodes) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>{t('admin:staffPriceCodes.title')}</CardTitle>
        </CardHeader>
        <CardContent>
    ...
    <>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between gap-2 pb-4">
          <CardTitle className="text-base sm:text-lg">{t('admin:staffPriceCodes.title')}</CardTitle>
          <Button size="sm" onClick={handleOpenCreateModal} className="shrink-0">
            <Plus className="h-4 w-4 sm:mr-2" />
            <span className="hidden sm:inline">{t('admin:staffPriceCodes.add')}</span>
          </Button>
        </CardHeader>
          ...
          {priceCodes && priceCodes.length > 0 ? (
            <>
              ...
              <div className="sm:hidden space-y-3">
                {priceCodes.map((pc: StaffPriceCode) => (
                  <div key={pc.id} className="border rounded-lg p-3 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div>
                        <span className="text-muted-foreground">{t('admin:staffPriceCodes.entryFee')}:</span>
                        <p className="font-medium">{formatCurrency(pc.entry_fee_brutto)}</p>
                      </div>
                      <div>
                        <span className="text-muted-foreground">{t('admin:staffPriceCodes.trainerFee')}:</span>
              ...
              <div className="hidden sm:block overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>{t('admin:staffPriceCodes.serviceType')}</TableHead>
                      <TableHead>{t('admin:staffPriceCodes.priceCode')}</TableHead>
                      <TableHead className="text-right">{t('admin:staffPriceCodes.entryFee')}</TableHead>
                      <TableHead className="text-right">{t('admin:staffPriceCodes.trainerFee')}</TableHead>
                      <TableHead>{t('admin:staffPriceCodes.validFrom')}</TableHead>
                      <TableHead>{t('admin:staffPriceCodes.status')}</TableHead>
            ...
            <p className="text-center text-muted-foreground py-8 text-sm">
              {t('admin:staffPriceCodes.noPriceCodes')}
            </p>
          )}
        </CardContent>
      </Card>
        ...
        <DialogContent className="w-[95vw] max-h-[90vh] overflow-y-auto sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle className="text-lg">
              {editingPriceCode
                ? t('admin:staffPriceCodes.editTitle')
                : t('admin:staffPriceCodes.createTitle')}
            </DialogTitle>
          </DialogHeader>
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            <div className="grid gap-3">
              ...
              <div className="grid gap-2">
                <Label>{t('admin:staffPriceCodes.serviceType')}</Label>
                <Select
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder={t('admin:staffPriceCodes.selectServiceType')} />
                  </SelectTrigger>
                  <SelectContent>
              ...
              {/* Price Code */}
              <div className="grid gap-2">
                <Label htmlFor="price_code">{t('admin:staffPriceCodes.priceCode')}</Label>
                <Input
                  id="price_code"
                  {...form.register('price_code')}
                  placeholder={t('admin:staffPriceCodes.priceCodePlaceholder')}
                  disabled={isPending}
                />
              </div>
              ...
              {/* Entry Fee */}
              <div className="grid gap-2">
                <Label htmlFor="entry_fee_brutto">{t('admin:staffPriceCodes.entryFee')}</Label>
                <Input
                  id="entry_fee_brutto"
                  type="number"
              ...
              {/* Trainer Fee */}
              <div className="grid gap-2">
                <Label htmlFor="trainer_fee_brutto">{t('admin:staffPriceCodes.trainerFee')}</Label>
                <Input
              ...
              {/* Valid From */}
              <div className="grid gap-2">
                <Label htmlFor="valid_from">{t('admin:staffPriceCodes.validFrom')}</Label>
                <Input
                  id="valid_from"
                  type="date"
              ...
              {/* Valid Until */}
              <div className="grid gap-2">
                <Label htmlFor="valid_until">{t('admin:staffPriceCodes.validUntil')}</Label>
                <Input
                  id="valid_until"
                  type="date"
      ...
      <Dialog open={!!deleteConfirmId} onOpenChange={() => setDeleteConfirmId(null)}>
        <DialogContent className="w-[95vw] sm:max-w-[400px]">
          <DialogHeader>
            <DialogTitle>{t('admin:staffPriceCodes.deleteConfirmTitle')}</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-muted-foreground">{t('admin:staffPriceCodes.deleteConfirmMessage')}</p>
          <DialogFooter className="flex-col sm:flex-row gap-2">
